...
```bash
agent-browser --session web-dev-server open http://localhost:5173/
agent-browser --session web-dev-server wait --load networkidle
agent-browser --session web-dev-server snapshot -i
```
...
If the route requires login first, read `login-script.md`.
...
```bash
cat <<'EOF' | agent-browser --session web-dev-server eval --stdin
(() => JSON.stringify({
  href: window.location.href,
  hasDialog: Boolean(document.querySelector('[data-testid="modal-root"]')),
}))()
EOF
```
